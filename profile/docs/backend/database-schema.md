...
sequenceDiagram
    autonumber
    participant App as Backend (JPA)
    participant MF as Table: memo_file
    participant TRG as Function: trg_memo_file_count
    participant M as Table: memo

    Note over App, M: [첨부 파일 추가] 연관된 메모의 파일 개수 자동 증가
    App->>MF: INSERT INTO memo_file
    MF-->>TRG: Trigger: AFTER INSERT
    activate TRG
    TRG->>M: UPDATE file_count = file_count + 1
    M-->>TRG: (updated_at = now())
    deactivate TRG
    MF-->>App: Insert 완료

    Note over App, M: [첨부 파일 삭제] 연관된 메모의 파일 개수 자동 감소
    App->>MF: DELETE FROM memo_file
    MF-->>TRG: Trigger: AFTER DELETE
    activate TRG
    TRG->>M: UPDATE file_count = GREATEST(file_count - 1, 0)
    M-->>TRG: (updated_at = now())
    deactivate TRG
    MF-->>App: Delete 완료
```
